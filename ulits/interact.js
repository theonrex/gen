...
export const getNumberMinted = async () => {
  const tokenMinted = await nftContract.methods.tokenMinted().call()
  return tokenMinted
}

export const getMaxSupply = async () => {
  const MAX_NFT_SUPPLY = await nftContract.methods.MAX_NFT_SUPPLY().call()
  return MAX_NFT_SUPPLY
}

export const isPausedState = async () => {
  const paused = await nftContract.methods.pauseMint().call()
  return paused
}
...
export const publicMint = async (mintAmount) => {
  ...
  try {
    ...
    return {
      success: true,
      status: (
        <a className='px' href={`https://polygonscan.com/address//${txHash}`} target="_blank">
